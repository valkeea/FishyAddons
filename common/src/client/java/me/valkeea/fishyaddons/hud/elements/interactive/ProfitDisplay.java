...
public class ProfitDisplay extends InteractiveHudElement {
    ...
    private List<Component> getDisplayLines(CachedHudData displayData, int color) {
        ...
        lines.add(Component.literal("Profit Tracker").withStyle(style -> style.withColor(titleColor))
            .append(Component.literal(displayData.timeString + profileSuffix)));
        ...
        lines.add(
            Component.literal(("Items: ")).withStyle(style -> style.withColor(titleColor))
                .append(Component.literal(String.valueOf(displayData.totalItems))
                .withStyle(style -> style.withColor(Color.brighten(color, 0.6f)))));

        if (displayData.totalValue > 0) {
            lines.add(Component.literal("Value: ").withStyle(style -> style.withColor(titleColor))
                .append(Component.literal(HudUtils.formatCoins(displayData.totalValue))
                .withStyle(style -> style.withColor(Color.brighten(color, 0.6f)))));
        }
    ...
    private Component getItemLine(ItemValueData data, int color) {

        var itemName = enhance(data.itemName);
        int quantity = data.quantity;
        var lineText = Component.literal(String.format("+%d ", quantity))
            .withStyle(style -> style.withColor(Color.desaturateAndDarken(color, 0.3f)))
            .append(Component.literal(itemName).withStyle(style -> style.withColor(Color.desaturate(color, 0.8f))));

        if (ProfitTracker.pricePerItem() && data.totalValue > 0) {
            lineText.append(Component.literal(" §7("))
                .append(Component.literal(HudUtils.formatCoins(data.totalValue)).withStyle(style -> style.withColor(Color.brighten(color, 0.6f))))
                .append("§7)");
        }
    ...
    protected List<Component> getLineTooltip(int lineIndex, Component line) {
        if (isLineClickable(lineIndex, line) && line.getString().startsWith("+")) {
            return List.of(
                ...
                Component.literal("§7Item value will still be counted towards total profit."),
                Component.literal("§7Use §3/fp ignored §7to view or restore ignored items.")
            );
            }
        return List.of();
    }
    ...
    protected void handleLineClick(Component line) {
        ...
        FishyNotis.send(Component.literal("§bHiding item from tracker: §b" + displayName
        + " §b[§3/fp restore§b]"));
    }
    ...
    public void restoreExcludedItem(String itemName) {
        ...
        var displayName = enhance(itemName);
        FishyNotis.send(Component.literal("§aRestored item: §f" + displayName));
    }
    ...
    private List<Component> placeHolderLines() {
        List<Component> lines = new ArrayList<>();
        lines.add(Component.literal("Profit Tracker (5m)"));
        lines.add(Component.literal("§b+3 §fRecombobulator 3000"));
        lines.add(Component.literal("Items: §3"));
        lines.add(Component.literal("Value: §b~27m"));
        return lines;
    }
}
