...
public class DianaStats {
    ...
    private int sessionBurrows = 0;
    private int sessionMobs = 0;
    private int sinceInq = 0;
    private int mobSinceInq = 0;
    ...
    public void saveSession() {
        ...
        StatConfig.setDiana(BU_BURROWS, sessionBurrows);
        StatConfig.setDiana(BU_SESSIONSTART, (int)sessionStartTime);
        StatConfig.setDiana(BU_MOBS, sessionMobs);
        StatConfig.setDiana(BU_SINCE_INQ, sinceInq);
        StatConfig.setDiana(BU_MOB_SINCE_INQ, mobSinceInq);
    }
    ...
    private void burrows() {
        ...
        ActivityMonitor.getInstance().recordActivity(Currently.DIANA);
        sessionBurrows++;
        sinceInq++;
    }
    ...
    public void save() {
        ...
        int totalBurrows = StatConfig.getDiana(TOTAL_BURROWS_KEY, 0);
        StatConfig.setDiana(TOTAL_BURROWS_KEY, totalBurrows + sessionBurrows);
        ...
        StatConfig.setDiana(SINCE_INQ_KEY, sinceInq);

        int totalMobs = StatConfig.getDiana(TOTAL_MOBS_KEY, 0);
        StatConfig.setDiana(TOTAL_MOBS_KEY, totalMobs + sessionMobs);

        StatConfig.setDiana(MOB_SINCE_INQ_KEY, mobSinceInq);
    ...
    public int getMobSinceInq() {
        return mobSinceInq;
    }

    // --- Getters for totals (session + persistent) ---
    
    public int getTotalBurrows() {
    ...
    public int getTotalMobs() {
        return StatConfig.getDiana(TOTAL_MOBS_KEY, 0) + sessionMobs;
    }
    ...
    public double getSessionBurrowsPerHour() {
        ...
        return ((double)sessionBurrows / sessionMinutes) * 60.0;
    }

    public double getTotalBurrowsPerHour() {
        double totalMinutes = getTotalPlaytimeMinutes();
    ...
    public double getAvgBurrowsPerInq() {
        double totalInq = getTotalInqCount();
    ...
    private boolean isCurrentlyActive() {
        return ActivityMonitor.getInstance().isActive(Currently.DIANA);
    }
