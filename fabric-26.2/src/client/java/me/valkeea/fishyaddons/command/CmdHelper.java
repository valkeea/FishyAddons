...
public class CmdHelper {
    ...
    private static char getFirstColorCode(Component text) {
        ...
        if (color != null) {
            ...
            if (formatting != null) code = "0123456789abcdefklmnor".charAt(formatting.ordinal());
        }
    ...
    private static ChatFormatting getFormattingFromTextColor(TextColor color) {
        ...
        for (var candidate : ChatFormatting.values()) {
            var legacyColor = TextColor.fromLegacyFormat(candidate);
            if (legacyColor != null && legacyColor.getValue() == rgb) {
                return candidate;
            }
        }
